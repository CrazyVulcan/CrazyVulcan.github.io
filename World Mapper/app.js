...
const WORLD_TABLES = {
  themes: ["Civilized", "Frontier", "Industrial", "Religious", "Haunted"],
  centerTypes: [
...
const THEME_FLAVOR = {
  Civilized: ["strict etiquette governs outcomes", "public reputation decides who gets heard", "guild procedure can override common sense"],
  Frontier: ["land claims change faster than maps", "law depends on who can enforce it", "every decision can start a local feud"],
  Industrial: ["production quotas pressure every office", "labor disputes can halt an entire district", "investors reward speed over caution"],
  Religious: ["oath and doctrine shape public trust", "ritual calendars alter civic priorities", "clergy endorsements sway major decisions"],
  Haunted: ["old spirits unsettle normal negotiations", "unexplained omens distort public fear", "the dead are treated like active witnesses"],
};

const DISTRICT_QUIRKS = {
  Farming: ["harvest rights are openly contested", "seed stores are tightly controlled", "livestock prestige affects social rank"],
  Trade: ["contracts are judged in public disputes", "price swings trigger faction blame", "caravan routes carry political leverage"],
  Housing: ["neighborhood factions patrol their own streets", "rent rules hide private rivalries", "resident councils control access to services"],
  Manor: ["old families broker influence through ceremonies", "inheritance conflicts spill into policy", "private galas function as negotiation tables"],
  Tower: ["watch protocols decide who is investigated", "signal schedules reveal hidden priorities", "inspection records are politically sensitive"],
  Shops: ["stall placement determines who survives", "quality seals are weaponized in rivalries", "license renewals can be quietly blocked"],
  Factory: ["output targets trump safety concerns", "sabotage rumors spread between shifts", "foremen control scarce advancement slots"],
  Guilds: ["charter clauses are exploited in power plays", "apprentice votes can swing leadership", "committee minutes hide real alliances"],
  Mansion: ["estate influence outweighs formal office", "household debts steer public decisions", "salons become unofficial courts"],
  University: ["commission panels decide academic futures", "research patronage shapes truth", "department feuds spill into city politics"],
  Palace: ["protocol determines petition access", "court rivalries slow critical orders", "ceremonial rank blocks practical action"],
  Fortress: ["security drills mask political purges", "supply ledgers reveal hidden agendas", "command rotations change local law overnight"],
  Temple: ["public vows are used as leverage", "relic custody confers authority", "doctrinal disputes split key allies"],
  Historic: ["heritage claims justify modern power", "artifact custody triggers legal battles", "restoration funding rewrites local narratives"],
  "Guild HQ": ["charter archives contain dangerous secrets", "votes are traded for protection", "legacy members block reform"],
  "Seat of Power": ["advisors manipulate access to decrees", "succession gossip destabilizes policy", "state rituals mask urgent crises"],
  Docks: ["berth assignments determine who profits", "manifests vanish at convenient times", "harbor strikes can paralyze governance"],
  Warehouse: ["stock records hide contraband routes", "priority seals decide who goes hungry", "night shifts answer to unofficial bosses"],
  Bank: ["credit committees pick winners", "debt pressure turns into political control", "audit findings are selectively leaked"],
};

const HOOK_SENTENCE_TEMPLATES = [
  "At the {specific} {broadType} district, a {motivationLower}-driven contact needs your help to {intent} because {themeFlavor} and {districtQuirk}.",
  "A local fixer at {specific} wants you to {intent}; in this {theme} center, {themeFlavor}, and {districtQuirk}.",
  "Your lead in {specific} asks for discreet help to {intent}: {districtQuirk}, and {themeFlavor}.",
  "Mission brief: {intent} from within {specific} ({broadType}) where {themeFlavor}; meanwhile, {districtQuirk}.",
];
...
function comboHookSentence(theme, broadType, specific, motivation, rng) {
  ...
  const themeFlavors = THEME_FLAVOR[theme] || ["local conditions are unstable"];
  const districtQuirks = DISTRICT_QUIRKS[broadType] || ["stakes are rising by the hour"];
  const template = rng.pick(HOOK_SENTENCE_TEMPLATES);
  const tokens = {
    theme,
    broadType,
    specific,
    motivationLower: motivation.toLowerCase(),
    intent: rng.pick(intents),
    themeFlavor: rng.pick(themeFlavors),
    districtQuirk: rng.pick(districtQuirks),
  };
...
function buildRegion(seed, centerCount) {
  ...
  for (let c = 1; c <= centerCount; c++) {
    ...
    const centerTheme = rng.pick(WORLD_TABLES.themes);
    const center = {
      id: `center-${c}`,
    ...
    center.districts = centerType.letters.map((letter, i) => {
      ...
      const district = {
        ...
        broadType,
        specific,
        tags: [slug(centerTheme), slug(centerType.name), slug(letter), slug(broadType), slug(specific)].filter(Boolean),
  ...
  return {
    ...
    themesUsed: [...new Set(centers.map((c) => c.theme))],
    centers,
    districts,
  };
}
...
els.generateBtn.addEventListener("click", () => {
  try {
    ...
    renderMap();
    renderDetail();
    log(`Generated ${centerCount} population center(s) across themes: ${state.region.themesUsed.join(", ")}.`);
  } catch (e) {
